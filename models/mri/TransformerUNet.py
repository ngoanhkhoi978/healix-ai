...
from models.mri.EncoderDecoder import ConvBlock, EncoderLayer, DecoderLayer
...
class TransformerUNet(nn.Module):
    def __init__(self, channels: Tuple[int], num_heads=2, is_residual: bool = False, bias=False) -> None:
        ...
        self.decode = nn.ModuleList([DecoderLayer(channels[i + 1], channels[i], is_residual, bias) for i in
                                     reversed(range(1, len(channels) - 1))])
...
class MultiHeadCrossAttention(nn.Module):
    def __init__(self, embed_dim: int, num_heads: int, channel_S: int, channel_Y: int, bias=False) -> None:
        ...
        nn.init.xavier_uniform_(self.upsample[0].weight.data)
